...
useEffect(() => {
  ...
  useEffect(() => {
    setFieldsValidated(username !== "" && password !== "" && confirmPassword !== "" && password === confirmPassword);
  ...
  const fetchPokemons = async () => {
    ...
    for (const id of pokemonNames) {
      const resp = await fetch(`https://studydex.onrender.com/student/pokemon/${id}`);
      const result = await resp.json();
      const pokemon = { name: result.name, sprite_url: result.sprite_url, id: result.pokemon_id };
      pokemonData.push(pokemon);
    }
    setPokemons(pokemonData);
  };
  const addChosenPokemon = async (id) => {
    const resp = await fetch(`https://studydex.onrender.com/student/pokemon/new/${id}`, {
      method: 'POST',
      headers: {
  ...
  useEffect(() => {
    ...
    <div className="modal fade" id="chooseYourPokemon" aria-hidden="true" aria-labelledby="chooseYourPokemonLabel" tabIndex="-1">
  <div className="modal-dialog modal-dialog-centered">
    <div className="modal-content">
      <div className="modal-header">
        <h1 className="modal-title fs-5" id="chooseYourPokemonLabel">Choose your fighter!</h1>
      ...
      <div className="modal-body">
        ...
        <div className="row">
          ...
          <div className="col" key={pokemon.id}>
            <div id={pokemon.id} className={`${selected && selected.id === pokemon.id ? 'selected' : ''} d-flex align-items-center flex-column`} onClick={() => handlePokemonClick(pokemon)}>
